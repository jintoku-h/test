...
def main_process():
    ...
    parameters = ak.parse_float_csv('./config/robot_param.csv') #ここにティーチング座標を入力したcsvファイルの名前
    ...
    try:
        ...
        ser = [ak.PwDispenserSDB1(com_port['powder_disp0_port'], firmware=3.7), 
        ...
        for i in range(int(table[0][0])):
            for j in range(int(table[1][0])):
                ...
                #Z軸ステージ運搬&ホモジナイザー操作
                dobot.place_zstage(z_stage_position, zstage, -8200)
